...
@MainActor
final class OSAWeekInLifeSimulationTests: XCTestCase {
    private var app: XCUIApplication!
    private var reporter: WeekSimulationReporter!
    private var pinnedQuickCardTitle: String = "Boil Water Advisory Steps"
    ...
    private func launch(dayIndex: Int, connectivity: WeekSimulationConnectivity, resetState: Bool = false) {
        app?.terminate()
        app = launchWeekSimulationApp(dayIndex: dayIndex, connectivity: connectivity, resetState: resetState)
    }

    private func relaunch(dayIndex: Int, connectivity: WeekSimulationConnectivity) {
        app?.terminate()
        app = launchWeekSimulationApp(dayIndex: dayIndex, connectivity: connectivity, resetState: false)
    }
    ...
    private func runStep(
        ...
        do {
            ...
            reporter.recordEvent(day: day, message: "\(feature) failed: \(error.localizedDescription)", severity: failureSeverity)
            return reporter.recordStep(
                day: day,
                feature: feature,
                action: action,
    ...
    private func addInventoryItem(_ fixture: InventoryFixture) throws {
        app.tapTab("Inventory")
        app.openInventoryAddForm()
    ...
    private func completeVisibleQuiz() throws {
        var safetyCounter = 0
    ...
    private func setToggle(_ toggle: XCUIElement, isOn: Bool) {
        ...
        for _ in 0..<3 {
            ...
            if control.isHittable {
                ...
                // Some SwiftUI form toggles expose a composite row to XCTest; the
                // trailing edge tap is the fallback when the switch itself is not hittable.
    ...
    private func toggleControlElement(for toggle: XCUIElement) -> XCUIElement {
    ...
    private func toggleValue(for toggle: XCUIElement) -> String {
    ...
    private func scrollToHittableElement(_ element: XCUIElement, maxSwipes: Int = 6) -> Bool {
