...
3. ~~Implement trusted-source HTTP client with fetch guards.~~ **Complete:** M4P3 `URLSessionTrustedSourceHTTPClient` enforces connectivity, HTTPS, allowlist, HTTP status, content-type, payload size, and post-redirect host validation. M4P4 import pipeline (normalization, chunking, local commit, index extension) is now complete. Next: M4P5 refresh and retry coordination.
