...
- M4P3 trusted-source allowlist and HTTP client are implemented: `TrustedSourceAllowlist` enumerates 17 PNW-focused approved publishers across three trust tiers (curated/approved, community/approved, unverified/pending). `TrustedSourceHTTPClient` protocol defines the fetch contract; `URLSessionTrustedSourceHTTPClient` provides the live implementation with connectivity, HTTPS, allowlist, status, content-type, size, and post-redirect guards. `TrustedSourceFetchResponse` carries raw fetch metadata and body.
...
1. ~~Define the first trusted-source allowlist and trust tiers.~~ **Complete:** Three-tier allowlist implemented in `TrustedSourceAllowlist` with 17 PNW-focused sources. Tier 1–2 curated/community with `.approved` status; Tier 3 unverified with `.pending` status. Exact host matching, no wildcards.
