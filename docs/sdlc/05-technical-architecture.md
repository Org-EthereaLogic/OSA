...
- The repository now contains an app shell with tab navigation, explicit `App/Bootstrap` and `App/Navigation` boundaries, a split shared design system, connectivity state modeling, domain-facing repository contracts for editorial and practice content, SwiftData models for handbook chapters, sections, quick cards, field references, and practice progress, plus bundled seed-import wiring for the offline content corpus through content-pack version `0.6.0`.
...
- Sprint 8 advanced maps and navigation is complete: `OSMTileCacheService` extended with explicit plan → save → list → delete tile-region management and `TileRegionBudget` enforcement. `CLLocationManagerService` extended to support continuous track recording with `AsyncStream`-based track-point emission. New domain layer in `OSA/Domain/Maps/` covers `UserWaypoint`, `RecordedTrack`, `RecordedTrackPoint`, `CachedTileRegion`, `TileRegionSavePlan`, and `CoordinateDisplayFormat`. New persistence layer: `PersistedWaypoint`, `PersistedRecordedTrack`, `PersistedRecordedTrackPoint`, `MapRecordMappings`, `SwiftDataWaypointRepository`, and `SwiftDataRecordedTrackRepository` in `OSA/Persistence/SwiftData/`. Shared utilities added to `OSA/Shared/Support/`: `NavigationSupport.swift` (`RescueCoordinateFormatter`, `NavigationDistanceCalculator`, `SunCompassCalculator`) and `GPXExporter.swift` (GPX 1.1 track and waypoint serialization). `MapScreen` enhanced with full waypoint, track recording, region management, compass, and distance-measurement surfaces.
- Sprint 10 multimedia and local practice is complete: quick cards and field references now carry optional bundled media attachments and quiz definitions through `ContentEnhancementModels`; `PracticeProgressRepository`, `PracticeProgressModels`, `PersistedPracticeProgress`, and `SwiftDataPracticeProgressRepository` keep quiz mastery and weekly-drill completion separate from editorial packs; `LocalSVGIllustrationView`, `LocalVideoPlayerView`, `ContentMediaSectionView`, `QuizSessionView`, and `CompletionBadgeStripView` extend existing Home, Quick Cards, and Library seams without introducing streaming or remote media dependencies.
- Sprint 12 internationalization and accessibility depth is complete: `AppLocalization` in `OSA/Shared/Localization/` provides runtime language switching backed by `Localizable.xcstrings` string catalog in `OSA/Resources/Localization/`. `AccessibilitySettings` extended with `appLanguageKey` (English/Spanish) and `highContrastModeKey`. `AppLanguage` enum supports `.english` and `.spanish` with locale and display-name accessors. `ColorTokens` extended with bounded high-contrast surface, background, stroke, and hero-text variants (`osaReadableSurface(highContrast:)`, `osaReadableBackground(highContrast:)`, `osaReadableStroke(highContrast:)`, `osaHeroPrimaryText(highContrast:)`, `osaHeroSecondaryText(highContrast:)`, `osaHeroMetadataText(highContrast:)`). Bundled quick-card seed packs carry optional Spanish `titleEs` and `bodyEs` editorial fields plus translated `mediaAccessibilityLabelEs`. Touched reading surfaces (Home, Ask, Settings, Quick Cards, Emergency Mode, handbook detail, emergency protocols) consume localized strings and high-contrast tokens.
- Post-Sprint-12 hardening: `FallbackWeatherForecastService` now chains `WeatherKitForecastService` → `OpenMeteoForecastService` as a graceful degradation path when WeatherKit is unavailable (commit 5181c86). `OSATwoWeekSimulationTests` extends the week-in-life simulation to 14 days with Week 2 persona data covering routine maintenance, deeper research, map waypoints, concurrent checklists, export flows, emergency drill, and comprehensive state audit. Updated automated test count: 577 total tests (448 XCTest unit + 56 XCTest UI + 73 Swift Testing).
- Sprint 11 advanced device capabilities is complete: knowledge pack management via `KnowledgePackCatalogClient` (approved-host validation, bundled/remote catalog routing) and `KnowledgePackDownloadCoordinator` (versioned install with retry) in `Networking/Packs/`; `KnowledgePackInstallStateRepository` with `PersistedKnowledgePackInstallState` SwiftData model; `KnowledgePackManagementScreen` in Settings. Document vault with encrypted file storage: `DocumentVaultEntry` domain model with `DocumentVaultCategory` (identity, medical, insurance, property, finance, emergencyPlan, other) and `DocumentCaptureSource` (camera, photoLibrary, fileImport); `DocumentVaultKeyStore` in `Persistence/Security/` stores 256-bit CryptoKit SymmetricKey in Keychain with `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`; `EncryptedDocumentVaultStore` in `Persistence/FileStorage/`; new `documents` tab and `Features/Documents/` with vault screen, detail, and capture views. Inventory photo capture: `InventoryCaptureModels` (capture source, barcode scan, recognized text, photo attachment) in `Domain/Inventory/Models/`; `FileBackedInventoryPhotoStore` in `Persistence/FileStorage/` with atomic writes and file-protection enforcement.
...
- `Document Vault`: encrypted document storage for sensitive personal documents (identity, medical, insurance, property, finance, emergency plans). Category filtering, OCR summary display, and camera/library/file-import capture flows.
...
- `PracticeProgressRepository`: local quiz-progress and weekly-drill completion reads or writes for Home, Quick Cards, and Library detail flows.
...
- SwiftData entities for product state and normalized content, including dedicated editorial records for handbook chapters, handbook sections, quick cards, and field references plus a separate local `PersistedPracticeProgress` record for quiz and weekly-drill completion. Sprint 11 adds `PersistedKnowledgePackInstallState` for pack install tracking and `PersistedDocumentVaultEntry` for encrypted document metadata.
...
- `Persistence/Security/` provides `DocumentVaultKeyStore` for Keychain-backed encryption key management (256-bit CryptoKit SymmetricKey with `kSecAttrAccessibleWhenUnlockedThisDeviceOnly`).
- `Persistence/FileStorage/` provides `EncryptedDocumentVaultStore` for AES-GCM encrypted document files and `FileBackedInventoryPhotoStore` for inventory photo attachments with atomic writes and `FileProtectionType.completeUntilFirstUserAuthentication`.
...
- `Features/Practice`: local quiz session and completion-badge views reused by Home, Quick Cards, and Library detail surfaces.
- `Domain/Practice`: local progress models, weekly-drill scheduling helpers, and repository contracts for quiz mastery and weekly-drill completion.
...
- `App/SystemSurfaces/`: Sprint 7 widget-safe system-surface bridge. `WidgetSnapshotModels` defines bounded snapshot types (readiness, expiry, rotating tip, emergency actions). `WidgetSnapshotStore` writes derived snapshots to an App Group container. `WidgetSnapshotCoordinator` reads from existing repositories and writes to the store at appropriate update points without exposing the full SwiftData stack to the extension. `ActiveProtocolActivityAttributes` and `ActiveProtocolActivityMapper` define Live Activity state from `ChecklistRun` progress. `ProtocolLiveActivityCoordinator` orchestrates ActivityKit start/update/end calls. `SystemSurfaceDeepLink` and `SystemSurfaceConfiguration` define safe deep-link URL schemes. `SystemSurfaceRepositories` provides a lightweight read-only facade consumed by the coordinator.
- `OSAWidgetsExtension/` (extension target): Sprint 7 WidgetKit and ActivityKit extension. `ReadinessScoreWidget`, `NextExpiringItemWidget`, `RotatingTipWidget`, and `EmergencyAccessWidget` read from the App Group `WidgetSnapshotStore` — no direct SwiftData access. `ActiveProtocolLiveActivity` presents `ActiveProtocolActivityAttributes` state on the Lock Screen and Dynamic Island. `OSAWidgetsBundle` exports all widget families. Privacy enforced: extension reads only bounded snapshot types; Ask history, notes, and personal content are never surfaced.

The boundary matters more than the literal number of Xcode targets; the `OSAWidgetsExtension` is acceptable because it requires its own lifecycle and sandbox that the main app target cannot provide — but it must remain presentation-focused and consume only the bounded snapshot models, not the app's full persistence stack.
...
- Quick cards and field references can now carry bundled media attachments, quiz definitions, and weekly-drill metadata while remaining the same first-class editorial records already used by search, Library routing, citations, and Ask.
...
When fixture mode is `week-sim`, `AppDependencies` substitutes `WeekSimulationTrustedSourceHTTPClient`, `WeekSimulationWeatherForecastService`, `WeekSimulationWeatherAlertService`, and `WeekSimulationRSSDiscoveryService` — all backed by canned responses in `UITestWeekSimulationFixtures`. The week-simulation regression tests (`OSAWeekInLifeSimulationTests` for 7-day and `OSATwoWeekSimulationTests` for 14-day) exercise persona simulations through all major features with artifact collection (`timeline.json`, `summary.md`, screenshots). Both share infrastructure via `SimulationTestSupport` protocol and `WeekSimulationSupport` helpers. `scripts/run-week-simulation.sh` orchestrates the simulator lane.
