...
- `40-sprint-5-notifications-export-sharing-enhanced-prompt.md` captures the next bounded follow-up slice: user-opt-in local inventory-expiry alerts, checklist PDF export, inventory CSV export, note markdown and plain-text export, quick-card and handbook share sheets, a family emergency plan note template, stronger related-content links, and clearer Library content-type filtering without introducing backend or sync behavior.
